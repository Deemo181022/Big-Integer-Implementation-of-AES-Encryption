/*
Basic structure was taken from:
https://github.com/manvelmk/AES-Rijndael-Encryption/blob/master/AES.cpp
A few edits from this were made to get everything working
*/
...
// Contains the values given by x to the power (i-1) being powers of x ({02}) in
// the field Galois Field (28)
int Rcon[255] = {
...
// O(1) - constant 14 itereations for 256 bit key
void Expand_Keys() {
  int i, j;
...
void Inv_Mix_Columns() {
  ...
  for (i = 0; i < 4; i++) {
    ...
    state[0][i] = Multiply(x1, 0x0e) ^ Multiply(x2, 0x0b) ^ Multiply(x3, 0x0d) ^
                  Multiply(x4, 0x09);
    state[1][i] = Multiply(x1, 0x09) ^ Multiply(x2, 0x0e) ^ Multiply(x3, 0x0b) ^
                  Multiply(x4, 0x0d);
    state[2][i] = Multiply(x1, 0x0d) ^ Multiply(x2, 0x09) ^ Multiply(x3, 0x0e) ^
                  Multiply(x4, 0x0b);
    state[3][i] = Multiply(x1, 0x0b) ^ Multiply(x2, 0x0d) ^ Multiply(x3, 0x09) ^
                  Multiply(x4, 0x0e);
  }
}

// O(1) - always 14 rounds for 256 bit key
void Encrypt() {
  int i, j, round = 0;
